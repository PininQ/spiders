...
class Tool:
    ...
    rmtr = re.compile(
        '<td>|</td>|<tr>|</tr>|<tr class"alt">|<tr class="datelisthed">')
    rmtime1 = re.compile('<td align="Center" width="7%>.*?</td>')
    retime2 = re.compile('<td class="noprint" align="Center".*?>.*?</td>')
...
class University:
    ...
    def Login(self):
        ...
        img = imgres.content
        # 如果result目录不存在，则创建该目录
        # if not os.path.exists(app.config["UP_DIR"]):
        #     os.makedirs(app.config["UP_DIR"])
        #     os.chmod(app.config["UP_DIR"], 'rw')
        with open(os.path.dirname(__file__) + '/result/code.jpg', 'wb') as f:
            f.write(img)
        jpg = Image.open('{}/result/code.jpg'.format(
        ...
        data = {
            ...
            "TextBox1": self.student.pwd,
            "TextBox2": self.student.pwd,
            "txtSecretCode": code,
        ...
        pattern = re.compile('<form name="Form1".*?action=(.*?) id="Form1">',
                             re.S)
    ...
    def GetClass(self):
        ...
        tool = Tool()
        f = open(
            os.path.dirname(__file__) + '/result/zhengfang.txt',
            'w',
            encoding='utf-8')
        f.write(u'本学期课表：' + '\n')
        cnt = 1
    ...
    def GetGrade(self):
        ...
        for each in grades:
            ...
            f.write('\n')
            totup += float(each[u'绩点']) * float(each[u'学分'])
            totdown = totdown + float(each[u'学分'])
        f.write('\n' + u'平均绩点：' + '%.2f\t\t\t' % (totup / totdown) +
                u'总学分绩点：' + '%.2f\t\t\t' % totup + u'总学分：' +
                '%.2f\n' % totdown)
        f.close()
        print('Download grade succeed!')

    def GreadeTestResults(self):
        self.session.headers['Referer'] = self.baseurl + \
            '/xss_main.apsx?xh=' + self.student.user
...
if __name__ == "__main__":
    # 贺州学院教务管理系统
    url = 'http://113.16.218.72:8082/(hx00jp45k15hwh55msxpxwfs)'
    user = input('学号：')
